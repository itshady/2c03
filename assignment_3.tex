...
\begin{myalgo}{\AName{PrintPrefix}($\VAR{node},\VAR{string}$)}
  ...
  \IF{$\VAR{node}=\VAR{root}$}
    ...
    \FOR{$char \in \VAR{string}$}
      ...
      \ENDIF
    \ENDFOR
    \STATE \AName{PrintPrefix($currNode.left, currNode.data+""$)}
    \STATE \AName{PrintPrefix($currNode.right, currNode.data+""$)}
  \ELSE
    \STATE $\VAR{string} += node.data$
    \IF{$node.color = red$}
      \PRINT string
    \ENDIF

    \STATE \AName{PrintPrefix($node.left,string$)}
    \STATE \AName{PrintPrefix($node.right,string$)}
  \ENDIF
\end{myalgo}

You can call the method via \AName{PrintPrefix}($\VAR{root},""$). The complexity if $\sim N + k$ since the algorithm traverse the path of the given prefix which is $\sim N$. Then it traverses every path below it recursively. Anytime it gets to a red node, it prints out the path it took to get there, including the current node (note: it does not need to retraverse the graph backwards as it holds the path in a string). This second part is $~k$ as it checks only all characters after the prefix. 
